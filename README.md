...
JList was approached from the [models][1] up. By reviewing the required features of the project a set of data points were identified to best represent the need for the application. Understanding the data needs for the application at the start is always the best guide to inform later decisions.

***Jfile***
*JList required four data points:*
- **jfile**
    - A [file field][2] required to store the file path for the JSON file to be uploaded
...
- **upload_date**  
    - A [datetime][3] field required to store the timestamp of when the file was uploaded set to record at the objects creation.
- **user**
    - A [character][4] field that will store the username of the logged in user that uploads a file or anonymous if no user is authenticated.
- **privacy**
    - A character field with a [choices][5] option linked to a list of possible dropdown options. By using this method over a boolean field the application gains scalability to include more groups at a later time.
...
In Django the [View][6] is the controller in the MVC model. Django supports function based views as well as classed baed veiws. This project uses the classed based views.

With the data identified and represented in the models it is now possible to create controllers that will be linked by urls and templates to render data.
...
***JsonListView***
- Django generic view [ListView][7] linked to ```model_name``` in ```models```.
- Custom Queryset:
    - Queryset will be filtered by ```user.group```.
    - If the user is not authenticated and is not in the group ```JLIST_ADMIN``` then return a queryset that [excludes][8] all model objects whose privacy is marked as ```'PRIVATE'```. Else, return all model objects.
...
***JsonFileUploadView***
- Django generic view [CreateView][9] linked to model_name in models.
...
- Custom form validation:
    - Override [form_vaild][10] and set ```commit=False```
...
- The view will need to be able to do the following:
    ...
    - The ```response['Content-Length']``` will need to be set.
        - hint: [Content-Length][13]
        - hint: ```os.stat(file_path).st_size```
    - The ```response['Content-Disposition']``` will need to be set to the attached file.
        - hint: [Content-Disposition][14]
    - Then return the ```response```.

**Forms**

If not already done so create a Form.py in the application folder. This is where the [model form][15] for JList will be created and then imported into the Views.
...
Create a [Urls.py][17] that will be included in the config/urls.py. This url file will import the Views and connect them to be served. Setting the ```app_name``` is a handy way to keep url namespaces short and easy to handle.
...
Insure to set up the [Admin][18] to give the logged in user the ability to administrate the model objects.

**Templates**

Now that all the backend data has been determined all that remains are the [templates][19]. For this project there are only three templates, one base, and two more that will render the home display and form.
...
[1]: https://docs.djangoproject.com/en/2.1/topics/db/models/
[2]: https://docs.djangoproject.com/en/2.1/ref/models/fields/#filefield
[3]: https://docs.djangoproject.com/en/2.1/ref/models/fields/#datetimefield
[4]: https://docs.djangoproject.com/en/2.1/ref/models/fields/#choices
[5]: https://docs.djangoproject.com/en/2.1/ref/models/fields/#choices
[6]: https://docs.djangoproject.com/en/2.1/topics/class-based-views/
[7]: https://docs.djangoproject.com/en/2.1/ref/class-based-views/generic-display/#listview
[8]: https://docs.djangoproject.com/en/2.1/ref/models/querysets/#exclude
[9]: https://docs.djangoproject.com/en/2.1/ref/class-based-views/generic-editing/#createview
[10]: https://docs.djangoproject.com/en/2.1/ref/class-based-views/mixins-editing/#modelformmixin
[11]: https://docs.djangoproject.com/en/2.1/topics/class-based-views/intro/#using-class-based-views
[12]: https://stackoverflow.com/questions/7296642/django-understanding-x-sendfile
[13]: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Length#Syntax
[14]: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Disposition#As_a_response_header_for_the_main_body
[15]: https://docs.djangoproject.com/en/2.1/topics/forms/modelforms/#modelform
[16]: https://docs.djangoproject.com/en/2.1/ref/models/instances/#django.db.models.Model.clean
